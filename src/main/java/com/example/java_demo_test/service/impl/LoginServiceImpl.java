...
public class LoginServiceImpl implements LoginService{
	...
	public LoginResponse addAccount(Login login) {
		 ...
		 * ^ 表示 從開頭開始
		 * ?= 表示 positive lookahead，前面有^表示要找「開頭後方必須是」，至少一個
		 * .* 表示 前方任意字符出現0次或多次
		 *    . 表示 任意字符
		 *    * 表示 出現0次或多次
		 * [!@#$%^&*(),.?\":{}|<>] 表示 []所有特殊符號
	...
	public LoginResponse activeAccount(String account, String pwd) {
		...
		if (login.isActive() == true) {
			return new LoginResponse("此帳號已經生效");
		}
		//如果帳號存在，就將boolean isActive設定成true，並存入資料庫
		login.setActive(true);
		
		return new LoginResponse(loginDao.save(login), "生效帳號成功");
	}
...
//		return new LoginResponse("登入成功");
		...
		if (login == null) {
			return new LoginResponse("此帳號或密碼不存在");
		}
		//確認: 帳號是否已經生效(還沒生效無法登入)
	...
	public LoginResponse findAccountByCity(String city) {
		...
		if (result.isEmpty()) {
			return new LoginResponse("無居住在此城市的使用者");
		}
		return new LoginResponse(result, "取得資訊成功");
	}
